...
def answer_check(questions_and_answers, answers_and_explanations, answer_check):
    if answer_check == 'A' or answer_check == 'a' or answer_check == '1':
        ...
        print('Please write either A, B, C or D')
...
def score_change(r_o_w):
    if r_o_w == 1:
        ...
        print(f'\n{list_players[turns]} just won {question_value_chosen} points! {list_players[turns]} score is {score_players[turns]} points!\n')
    else:
        score_players[turns] = score_players[turns] - int(question_value_chosen)
        print('\nThe answer is incorrect!',answers_and_explanations_to_show[1])
        print(f'\n{list_players[turns]} just lost {question_value_chosen} points! {list_players[turns]} score is {score_players[turns]} points!\n')
...
while questions_answered < number_of_questions:
    ...
    team_answer = str(input('What is your answer? Option A, B, C or D?: '))
    
    score_change(answer_check(question_and_answers_to_show, answers_and_explanations_to_show, team_answer))
        
    questions_answered += 1
    turns += 1
    
    board_print(category_chosen.upper(),question_value_chosen)
